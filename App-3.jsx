...
export default function WorkshopSite() {
  ...
  return (
    ...
    >
      ...
      {VIEW_PLANET[view] && (
        <div className="relative h-40 sm:h-56 overflow-hidden" aria-hidden="true">
          <Planet3D
            textureUrl={VIEW_PLANET[view].textureUrl}
            ringTextureUrl={VIEW_PLANET[view].ringTextureUrl}
            glowColor={VIEW_PLANET[view].glowColor}
          />
          <div
            className="absolute inset-x-0 bottom-1 text-center text-[10px] tracking-wide"
            style={{ color: "#6B7590", fontFamily: "'IBM Plex Mono', monospace" }}
          >
            {VIEW_PLANET[view].label}
          </div>
        </div>
      )}
      ...
      <section className="relative max-w-5xl mx-auto px-5 pt-12 pb-8 flex flex-col items-center text-center overflow-hidden">
        ...
        {/* Real 3D rotating Earth, anchored at the bottom of the hero */}
        <div
          className="absolute inset-x-0 bottom-0 h-52 sm:h-72 overflow-hidden pointer-events-none"
          style={{ zIndex: 5 }}
          aria-hidden="true"
        >
          <Earth3D />
      ...
      <footer className="border-t border-[#2A3B5C] py-6 text-center text-[10px] text-[#6B7590]">
        ...
        <div className="mt-1 text-[9px] opacity-70">
          Planet textures © Solar System Scope (CC-BY 4.0) — based on NASA imagery
        </div>
      </footer>
      </div>
    </div>
...
function LessonNode({ node, isAdmin, onEdit, onAddChild, onDelete, onMove, topLevel }) {
  ...
  const wrapperStyle = topLevel
    ? { backgroundColor: "#101830", border: "1px solid #2A3B5C" }
    : {};
...
function Earth3D() {
  const mountRefBox = useState(() => ({ current: null }))[0];

  useEffect(() => {
    const mount = mountRefBox.current;
  ...
  return <div ref={(el) => (mountRefBox.current = el)} className="absolute inset-0" />;
}

const SSS_TEX = (name) => `https://www.solarsystemscope.com/textures/download/2k_${name}.jpg`;

const VIEW_PLANET = {
  social: { textureUrl: SSS_TEX("mars"), glowColor: 0xd97a4a, label: "المريخ" },
  workshops: {
    textureUrl: SSS_TEX("saturn"),
    ringTextureUrl: "https://www.solarsystemscope.com/textures/download/2k_saturn_ring_alpha.png",
    glowColor: 0xd9c088,
    label: "زحل",
  },
  content: { textureUrl: SSS_TEX("jupiter"), glowColor: 0xd9a35a, label: "المشتري" },
  heroesList: { textureUrl: SSS_TEX("uranus"), glowColor: 0x7fd9d9, label: "أورانوس" },
  heroProfile: { textureUrl: SSS_TEX("uranus"), glowColor: 0x7fd9d9, label: "أورانوس" },
  certificates: { textureUrl: SSS_TEX("neptune"), glowColor: 0x4a6fd9, label: "نبتون" },
};

function Planet3D({ textureUrl, ringTextureUrl, glowColor = 0x4a90d9, rotationSpeed = 0.0009 }) {
  const mountRefBox = useState(() => ({ current: null }))[0];
  ...
  useEffect(() => {
    ...
    const material = new THREE.MeshPhongMaterial({ map: planetTexture, shininess: 6 });
    const planet = new THREE.Mesh(geometry, material);
    planet.position.set(0, -1.35, 0);
    scene.add(planet);

    let ring = null;
    let ringTexture = null;
    if (ringTextureUrl) {
      const ringGeometry = new THREE.RingGeometry(1.35, 2.15, 128);
      const pos = ringGeometry.attributes.position;
      const v3 = new THREE.Vector3();
      for (let i = 0; i < pos.count; i++) {
        v3.fromBufferAttribute(pos, i);
        ringGeometry.attributes.uv.setXY(i, v3.length() < 1.75 ? 0 : 1, 1);
      }
      ringTexture = textureLoader.load(ringTextureUrl);
    ...
    const animate = () => {
      ...
      if (ring) ring.rotation.z += rotationSpeed * 0.12;
      renderer.render(scene, camera);
      raf = requestAnimationFrame(animate);
    };
    animate();
    ...
    <>
      <div
        ...
        style={{
          zIndex: 0,
          background:
            `radial-gradient(ellipse 1000px 700px at 50% -10%, rgba(184,121,26,0.08) 0%, transparent 60%),` +
            `radial-gradient(ellipse 900px 600px at 20% 15%, rgba(${nebula.a},0.18) 0%, transparent 60%),` +
            `radial-gradient(ellipse 800px 700px at 85% 75%, rgba(${nebula.b},0.15) 0%, transparent 60%),` +
            `radial-gradient(ellipse at top, #0F1730 0%, #05070F 55%, #020306 100%)`,
          backgroundColor: "#020306",
        }}
      />
      <canvas ref={(el) => (canvasRefBox.current = el)} className="fixed inset-0 pointer-events-none" style={{ zIndex: 1 }} />
    </>
  );
}
