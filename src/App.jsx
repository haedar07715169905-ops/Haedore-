...
const STAT_PALETTE = [
  { bg: "#EAF6F1", border: "#BFE3D4", text: "#1F7A5C" },
  { bg: "#FBF1E0", border: "#EBCE96", text: "#B8791A" },
  { bg: "#FBEAE7", border: "#EFC3BC", text: "#B23A2E" },
  { bg: "#EAF0FB", border: "#C2D3F2", text: "#2554A8" },
];
...
const EMPTY_HERO_FORM = { name: "", bio: "" };
...
export default function WorkshopSite() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [w, s, c, st, cert, her] = await Promise.all([
  ...
  const openEditHero = (hero) => {
    setHeroForm({ name: hero.name, bio: hero.bio || "" });
  ...
  const submitHero = async (e) => {
    ...
    let next;
    if (heroEditId) {
      next = heroes.map((h) => (h.id === heroEditId ? { ...h, name: heroForm.name.trim(), bio: heroForm.bio } : h));
    } else {
      const newHero = { id: "hero" + Date.now(), name: heroForm.name.trim(), bio: heroForm.bio, photoUrl: "", links: [] };
      next = [...heroes, newHero];
    }
  ...
  const openNodeEditor = (mode, targetId = null, parentId = null, existingNode = null) => {
    ...
    if (existingNode) {
      const kind = existingNode.isProfile
        ? "profile"
  ...
  return (
    <div
      ...
      style={{
        ...
        background: "radial-gradient(ellipse at top, #FDFBF6 0%, #FFFFFF 45%, #FFFFFF 100%)",
      }}
    ...
    >
      ...
      <section className="relative max-w-5xl mx-auto px-5 pt-12 pb-8 flex flex-col items-center text-center overflow-hidden">
        {/* Decorative background: original blueprint-style prosthetic limb illustrations */}
        ...
        <div
          className="relative w-40 h-40 sm:w-48 sm:h-48 mb-6 rounded-full"
          style={{
            boxShadow: "0 0 0 1px #F0E3BE, 0 8px 30px rgba(184,121,26,0.18)",
          }}
        >
          <img
            src={LOGO_SRC}
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-8">
          <button
            ...
            style={{ backgroundColor: "#FFFFFF", border: "1px solid #E4E1D8", boxShadow: "0 2px 10px rgba(31,122,92,0.10)" }}
          >
            <span
            ...
            <span style={{ color: "#1F2430" }}>حساباتنا على مواقع التواصل الاجتماعي</span>
          </button>
          <button
            onClick={() => setView("workshops")}
            className="group flex items-center gap-3 pl-5 pr-3 py-3 rounded-2xl text-sm font-semibold transition-all hover:-translate-y-1"
            style={{ backgroundColor: "#FFFFFF", border: "1px solid #E4E1D8", boxShadow: "0 2px 10px rgba(184,121,26,0.12)" }}
          >
            <span
              className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-105"
            ...
            <span style={{ color: "#1F2430" }}>ورشنا ودوراتنا التدريبية</span>
          </button>
          <button
            onClick={() => setView("content")}
            className="group flex items-center gap-3 pl-5 pr-3 py-3 rounded-2xl text-sm font-semibold transition-all hover:-translate-y-1"
            style={{ backgroundColor: "#FFFFFF", border: "1px solid #E4E1D8", boxShadow: "0 2px 10px rgba(178,58,46,0.12)" }}
          >
            <span
              className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-105"
            ...
            <span style={{ color: "#1F2430" }}>محتوياتنا</span>
          </button>
        </div>
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-1 mt-2">
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#1F2430] hover:border-[#E8A33D] bg-[#F8F7F2]"
                      >
                        <ChevronUp size={12} />
                      </button>
                      <button
                      ...
                      {confirmDelete && confirmDelete.type === "stat" && confirmDelete.id === stat.id ? (
                        ...
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#C1502E] hover:border-[#C1502E] bg-[#F8F7F2]"
                        >
                          <Trash2 size={12} />
                        </button>
                      )}
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      ...
                      {w.status === "ended" ? (
                        <div
                          ...
                          style={{ backgroundColor: "#131C30", color: "#6B7280" }}
                        >
                ...
                <div key={hero.id} className="flex flex-col items-center gap-2 relative">
                  ...
                  >
                    <div
                      ...
                      style={{ backgroundColor: "#F8F7F2", border: "2px solid #E8A33D" }}
                    >
                      {hero.photoUrl ? (
                        <img src={hero.photoUrl} alt={hero.name} className="w-full h-full object-cover" />
                      ) : (
                        <Users size={28} className="text-[#D9D4C6]" />
                      )}
                    </div>
      ...
      {view === "heroProfile" && selectedHeroId && (() => {
        ...
        return (
          <section className="max-w-3xl mx-auto px-5 pb-16 pt-10 flex flex-col items-center text-center">
            <div
              className="w-32 h-32 sm:w-40 sm:h-40 rounded-full overflow-hidden flex items-center justify-center mb-4"
              style={{ backgroundColor: "#F8F7F2", border: "3px solid #E8A33D" }}
            >
              {hero.photoUrl ? (
                <img src={hero.photoUrl} alt={hero.name} className="w-full h-full object-cover" />
              ) : (
                <Users size={44} className="text-[#D9D4C6]" />
              )}
            </div>

            {isAdmin && (
              <div className="flex items-center gap-2 mb-4">
                <label className="text-[11px] px-3 py-1.5 rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#1F2430] hover:border-[#E8A33D] cursor-pointer">
                ...
                {hero.photoUrl && (
                  <button
                    ...
                    className="text-[11px] px-3 py-1.5 rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#C1502E] hover:border-[#C1502E]"
                  >
                    حذف الصورة
                  </button>
                )}
            ...
            {isAdmin && (
              <button
                ...
                className="text-[11px] px-3 py-1.5 rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#1F2430] hover:border-[#E8A33D] mb-4 flex items-center gap-1"
              >
            ...
            {hero.bio && (
              <p
                className="text-sm text-[#4B5563] leading-loose mb-6 max-w-xl text-right"
                dir="rtl"
                style={{ whiteSpace: "pre-line" }}
              >
                {hero.bio}
            ...
            {hero.links.length === 0 ? (
              <div className="w-full border border-dashed border-[#D9D4C6] rounded-xl p-6 text-center text-[#6B7280] text-xs">
      ...
      {view === "certificates" && isAdmin && (
        <section className="max-w-5xl mx-auto px-5 pb-16 pt-10 scroll-mt-20">
          ...
          >
            ...
            {certPreviewUrl && (
              <div className="mt-4">
                <p className="text-xs text-[#6B7280] mb-2">معاينة حية (باسم تجريبي):</p>
                <img
                  src={certPreviewUrl}
                  alt="معاينة الشهادة"
            ...
            {bulkResults.length > 0 && (
              <div className="mt-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs text-[#6B7280]">تم توليد {bulkResults.length} شهادة</p>
                  <button
                  ...
                  {bulkResults.map((r) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-[10px] py-1 rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#1F2430] hover:border-[#E8A33D]"
                      >
                        تحميل
                      </button>
                    </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="text-[11px] px-3 py-1.5 rounded-md border border-[#D9D4C6] text-[#6B7280] hover:text-[#1F2430] hover:border-[#E8A33D]"
                    >
                      عرض QR
                    </button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-5 py-8 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setStatFormOpen(false)} className="text-[#6B7280] hover:text-[#1F2430]">
                <X size={18} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-5 py-8 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setHeroFormOpen(false)} className="text-[#6B7280] hover:text-[#1F2430]">
                <X size={18} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-5 py-8 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setHeroLinkFormOpen(false)} className="text-[#6B7280] hover:text-[#1F2430]">
                <X size={18} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-5 py-8 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setCertFormOpen(false)} className="text-[#6B7280] hover:text-[#1F2430]">
                <X size={18} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-5 py-8 overflow-y-auto">
          ...
          >
            <div className="flex items-center justify-between w-full mb-4">
              ...
              <button onClick={() => setCertQrFor(null)} className="text-[#6B7280] hover:text-[#1F2430]">
                <X size={18} />
              </button>
            </div>
          ...
          >
            {verifiedCert.status === "found" ? (
              <>
                ...
                <p className="text-xs text-[#6B7280] mb-1">اسم المشارك:</p>
                <p className="text-base font-semibold mb-4" style={{ color: "#B8791A" }}>
                  {verifiedCert.cert.name}
                </p>
                <p className="text-xs text-[#6B7280] mb-1">رقم التسلسل:</p>
              ...
              <>
                ...
                <p className="text-xs text-[#6B7280] mb-5">تأكد من الرمز أو تواصل مع منصة نَبض.</p>
              </>
            )}
            <button
    ...
    return (
      <div className={topLevel ? wrapperClass : "flex items-center gap-2"} style={wrapperStyle}>
        <div
          ...
          style={topLevel ? {} : { backgroundColor: "#0C1322", border: "1px dashed #D9D4C6", color: "#9CA3AF" }}
        >
          <Clock size={15} />
