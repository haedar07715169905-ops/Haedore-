...
export default function WorkshopSite() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [w, s, c, st, cert] = await Promise.all([
  ...
  const handleTemplateFileUpload = (e) => {
    ...
    reader.onload = async () => {
      try {
        const img = await loadImageEl(reader.result);
        const maxDim = 1100;
        ...
        if (dataUrl.length > 700000) {
          setCertUploadError("الصورة كبيرة جدًا حتى بعد الضغط، جرب صورة أبسط أو أصغر أبعادًا.");
          return;
        }
        setCertTemplateDraft((prev) => ({ ...prev, imageUrl: dataUrl }));
      } catch (err) {
