...
export default function WorkshopSite() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [w, s, c, st, cert, tmpl] = await Promise.all([
      ...
      setCertTemplate(SEED_CERT_TEMPLATE);
      setCertTemplateDraft(SEED_CERT_TEMPLATE);
    } finally {
      setLoaded(true);
    }
  }, []);
        ...
        <section className="max-w-5xl mx-auto px-5 pb-16 pt-10 scroll-mt-20">
          ...
          >
            ...
            <form onSubmit={submitCertTemplate} className="space-y-3">
              <Field label="رابط صورة قالب الشهادة">
                <input
                  value={certTemplateDraft.imageUrl}
                  onChange={(e) => setCertTemplateDraft({ ...certTemplateDraft, imageUrl: e.target.value })}
                  placeholder="https://... أو /images/certificate.jpg"
                  style={inputStyle}
                />
              </Field>
